...
# PHP client against it, for BOTH v2 (pinned, authenticated) and v1 (regression).
# Skips cleanly if the server binary isn't available.
set -u
PORT="${PORT:-3195}"
...
echo "--- v2 (pinned, authenticated) ---"; php "$HERE/e2e_v2.php" 127.0.0.1 "$PORT" "$PINNED"; rc2=$?
echo "--- v1 (regression) ---";            php "$HERE/e2e_v2.php" 127.0.0.1 "$PORT" v1;       rc1=$?
if [ $rc2 -eq 0 ] && [ $rc1 -eq 0 ]; then
  echo "END-TO-END PHP<->Rust PASSED (v2 authenticated + v1 unchanged)."; exit 0
else
